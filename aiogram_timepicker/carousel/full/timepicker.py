...
class TimePicker:
    ...
    async def start_picker(
        ...
        for i in range(self.count_top, 0, -1):
            ...
            t = _time + _tds[0] * i
            inline_kb.insert(InlineKeyboardButton(
                self.hour_format.format(t.hour),
                callback_data=self.callback.new('CHANGE', t.hour, minute, second)
            ))
            inline_kb.insert(InlineKeyboardButton(
                self.label_empty,
                callback_data=self.callback.new('IGNORE', hour, minute, second)
            ...
            t = _time + _tds[1] * i
            inline_kb.insert(InlineKeyboardButton(
                self.minute_format.format(t.minute),
                callback_data=self.callback.new('CHANGE', hour, t.minute, second)
            ))
            inline_kb.insert(InlineKeyboardButton(
                self.label_empty,
                callback_data=self.callback.new('IGNORE', hour, minute, second)
            ...
            t = _time + _tds[2] * i
            inline_kb.insert(InlineKeyboardButton(
                self.minute_format.format(t.second),
                callback_data=self.callback.new('CHANGE', hour, minute, t.second)
            ))
            inline_kb.insert(InlineKeyboardButton(
                self.label_empty,
                callback_data=self.callback.new('IGNORE', hour, minute, second)
        ...
        for i in range(1, self.count_bottom + 1):
            ...
            t = _time + _tds[2] * i
            inline_kb.insert(InlineKeyboardButton(
                self.second_format.format(t.second),
                callback_data=self.callback.new('CHANGE', hour, minute, t.second)
            ))
            inline_kb.insert(InlineKeyboardButton(
                self.label_empty,
    ...
    async def _process_selection_all_int(self, query: CallbackQuery, act: str, seconds: int, minutes: int, hours: int):
        if act == "CHANGE":
            await query.message.edit_reply_markup(
                await self.start_picker(hours, minutes, seconds)
            )
    ...
    async def process_selection(self, query: CallbackQuery, data: CallbackData) -> result.Result:
        ...
        if data['act'] == "CANCEL":
            ...
            return result.Result()
        if not (data['hours'].isdigit() and data['minutes'].isdigit() and data['seconds'].isdigit()):
            return await self._process_selection_not_int(
                query, data['act'], data['seconds'], data['minutes'], data['hours']
            )
        return await self._process_selection_all_int(
            query, data['act'], int(data['seconds']), int(data['minutes']), int(data['hours'])
        )
